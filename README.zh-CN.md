...
<p align="center"><strong>不要只照着用户的原话回答，找到真正需要解决的问题，然后把它解决。</strong></p>
...
照字面回答，只会比较价格、课程内容和评价。RightQuestion 会看出，真正需要解决的是如何把已经学会的知识转化为完成的作品。Agent 随后会给出找出实际阻力的具体办法，并据此判断是否真的需要购买新课程。

问题转化只是 Agent 的内部步骤；你最终得到的仍然是有用的答案或完成的工作。

## 它怎么工作

1. 理解你真正想得到的结果，而不只看你用了什么措辞。
2. 检查你假设的原因、已经想到的方案或有限的选项是否把任务带偏了。
3. 在内部重新确定真正要解决的问题，然后继续工作，直到给出答案或完成任务。

如果某个关键信息确实只能由你提供，Agent 会问一个必要的问题，得到回答后继续；它不会把自己能完成的分析重新丢给你。
...
它不再使用固定的三个标题。只有当一次重要的问题转化有助于你理解结果时，Agent 才会简单说明改变了什么，然后立刻继续解决。更深层的模型比较默认留在内部，除非你要求查看。

## Agent Skill 是什么

Agent Skill 可以理解为一份 AI 在需要时会自动拿出来使用的“做事方法”。RightQuestion 遵循开放的 [Agent Skills](https://agentskills.io/) 格式，可以在 Codex、Claude Code、Cursor 和 Gemini CLI 中使用。它本身不要求访问你的文件、账号或网络；具体任务需要的工具仍然遵守 Agent 原有的权限规则。
